...
#ifndef MIC_LIB_PATH
#error "MIC_LIB_PATH must be defined"
#endif
...
/* Add path to GCC accel libraries to MIC_LD_LIBRARY_PATH, which is required by
   liboffloadmic.  */
__attribute__((constructor))
static void
set_mic_lib_path (void)
{
  const char *mic_env = getenv (MIC_LD_LIBRARY_PATH_ENV);
  char *mic_env_new = (char *) malloc ((mic_env == NULL ? 0 : strlen (mic_env))
				       + strlen (MIC_LIB_PATH) + 2);
  if (mic_env == NULL)
    sprintf (mic_env_new, MIC_LIB_PATH);
  else
    sprintf (mic_env_new, "%s:"MIC_LIB_PATH, mic_env);
  setenv (MIC_LD_LIBRARY_PATH_ENV, mic_env_new, 1);
  free (mic_env_new);
}
