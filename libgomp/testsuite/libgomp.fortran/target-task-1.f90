...
module target_task_1_mod
  integer, parameter :: N = 10000000, CHUNKSZ = 100000
  real :: Y(N), Z(N)
end module target_task_1_mod

subroutine init ()
  use target_task_1_mod, only : Y, Z, N
  integer :: i
  do i = 1, N
    Y(i) = 0.1 * i
    Z(i) = Y(i)
...
subroutine pipedF ()
  use target_task_1_mod, only: Z, N, CHUNKSZ
  integer :: C, i
  real :: F
  do C = 1, N, CHUNKSZ
    !$omp task
...
subroutine pipedF_ref ()
  use target_task_1_mod, only: Y, N
  integer :: i
  real :: F
  do i = 1, N
    Y(i) = F (Y(i))
